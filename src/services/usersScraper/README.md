# Users Scraper
[Leetcode Roulette](https://leetcoderoulette.com) service that runs when a signin event is emitted to get [Leetcode](https://leetcode.com) user data.

## Getting Started
To get started running the Users Scraper service locally, clone the repository to your local machine and change to the project root directory. Next, make sure to make the needed configurations and run `npm install` to get the needed packages for the project. Next, build and run the project with `npm run build` followed by `npm start` respectively.

```
git clone https://github.com/leetcode-roulette/backend-services.git
cd backend-services/src/services/usersScraper
```
...
## Features
This project waits for `signin` events to be emitted and scrapes user data from [leetcode](https://leetcode.com) and emits them as events using [kafka](https://kafka.apache.org/). 
